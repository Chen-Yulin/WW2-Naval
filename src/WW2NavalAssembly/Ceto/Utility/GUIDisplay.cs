	...
	public class GUIDisplay : MonoBehaviour 
	{
		...
		void OnGUI() 
		{
            ...
            */
            ...
            if (spectrum != null)
			{
				float smoothing = spectrum.slopeSmoothing;

				GUILayout.BeginHorizontal("Box");
				GUILayout.Label("Slope Smoothing", GUILayout.MaxWidth(m_textWidth));
				smoothing = GUILayout.HorizontalSlider(smoothing, WaveSpectrum.MIN_SLOPE_SMOOTHING, WaveSpectrum.MAX_SLOPE_SMOOTHING);
				GUILayout.EndHorizontal();
				
				spectrum.slopeSmoothing = smoothing;
			}

			if(spectrum != null)
			{
				float smoothing = spectrum.foamSmoothing;

				GUILayout.BeginHorizontal("Box");
				GUILayout.Label("Foam Smoothing", GUILayout.MaxWidth(m_textWidth));
				smoothing = GUILayout.HorizontalSlider(smoothing, WaveSpectrum.MIN_FOAM_SMOOTHING, WaveSpectrum.MAX_FOAM_SMOOTHING);
				GUILayout.EndHorizontal();
		
				spectrum.foamSmoothing = smoothing;
			}
			*/
            ...
            if (underWater != null)
            {
                ...
                intensity = GUILayout.HorizontalSlider(intensity, 0.0f, UnderWater.MAX_ABSORPTION_INTENSITY);
                GUILayout.EndHorizontal();
